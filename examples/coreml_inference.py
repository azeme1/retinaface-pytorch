"""Run face detection with model.mlpackage (from this repo's scripts/
export_coreml.py, or downloaded from Hugging Face). macOS only (CoreML
runtime requirement) -- run on Linux/Windows via one of the other formats
in this directory instead.

    pip install coremltools opencv-python numpy
    python examples/coreml_inference.py --model model.mlpackage --image photo.jpg --image-size 640
"""
...
def main():
    ...
    p.add_argument("--image-size", type=int, default=640, help="must match the size the .mlpackage was exported with")
